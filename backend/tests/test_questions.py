...
def test_questions_can_be_retrieved():
    ...
    assert all(
        question.question_id and question.question_text
...
def test_topic_filtering_works():
    ...
    assert len(questions) > 0
    assert all(question.topic == "Python" for question in questions)
...
def test_topic_and_difficulty_filtering_works():
    questions = question_repository.get_by_topic_and_difficulty(
        "Python",
    ...
    assert all(
        ...
        and question.difficulty == DifficultyLevel.MEDIUM
        for question in questions
    )
...
def test_previously_asked_questions_are_not_selected_again():
    ...
    # This test requires at least two matching questions.
...
def test_multiple_previously_asked_questions_are_excluded():
    questions = question_repository.get_by_topic_and_difficulty(
        "Python",
        DifficultyLevel.EASY,
    )
    ...
    assert question is not None
    assert question.question_id not in asked_question_ids


def test_returns_none_when_all_matching_questions_were_asked():
    questions = question_repository.get_by_topic_and_difficulty(
        "Python",
        DifficultyLevel.EASY,
    )
...
def test_empty_asked_question_ids_works():
    question = select_next_question(
        topic="Python",
        difficulty=DifficultyLevel.EASY,
        asked_question_ids=[],
...
def test_questions_api_filters_by_topic():
    ...
    assert len(body) > 0
    assert all(question["topic"] == "OOP" for question in body)
...
def test_questions_api_filters_by_topic_and_difficulty():
    ...
    assert all(
        question["topic"] == "SQL"
        and question["difficulty"] == "medium"
...
def test_interview_start_assigns_a_question_when_topic_matches():
    ...
    assert body["current_question_id"] is not None
